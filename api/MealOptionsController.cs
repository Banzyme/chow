...
namespace ChowServerlessRestAPI
{
    ...
    {
        private const string MealsCollectionName = "mealsCollectionV2";
        ...
        [CosmosDB(
                databaseName: "chowDB",
                collectionName: MealsCollectionName,
        ...
        public static async Task<IActionResult> CreateNew(
                ...
                [CosmosDB(databaseName: "chowDB", collectionName: MealsCollectionName,
                ...
                ILogger log)
        {
            ...
            Uri collectionUri = UriFactory.CreateDocumentCollectionUri("chowDB", MealsCollectionName);
        ...
        public static async Task<IActionResult> Update(
            ...
            [CosmosDB(databaseName: "chowDB", collectionName: MealsCollectionName, ConnectionStringSetting = "CosmosDBConnection")] IDocumentClient cosmosDBClient,
            ILogger log)
        {
            ...
            Uri docCollectionUri = UriFactory.CreateDocumentCollectionUri("chowDB", MealsCollectionName);
            var docToUpdate = GetMealDocumentByMealId(cosmosDBClient, docCollectionUri, queryId);
            if (docToUpdate == null)
                return new NotFoundResult();

            Uri docUri = UriFactory.CreateDocumentUri("chowDB", MealsCollectionName, docToUpdate.Id);
            ...
            ILogger log)
        {
            ...
            var requestOpts = new RequestOptions() { PartitionKey = new PartitionKey(cat) };
            Uri docUri = UriFactory.CreateDocumentUri("chowDB", MealsCollectionName, docToDelete.Id);
        ...
        private static Document GetMealDocumentByMealId(IDocumentClient cosmosDBClient, Uri docCollectionUri, string mealId)
        {
            ...
            var feedOpts = new FeedOptions() { EnableCrossPartitionQuery = true };
